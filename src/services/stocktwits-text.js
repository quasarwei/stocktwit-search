...
(function () {
  stwt.txt = {};
  stwt.txt.regexen = {};
  stwt.txt.regexen.cashtag = /(^|[\s\,\.\-\+\(\/\"\']\$?|^\$)(\$([a-z1-9]{1}[a-z]{1,3}_F|(?!\d+[bmkts]{1}?(il(lion)?|ln|m|n)?\b|[\d]+\b)(?!\d+usd)[a-z0-9]{1,9}(?:[-\.]{1}[a-z]{1,2})?(?:[-\.]{1}[a-z]{1,2})?))\b(?!\$)/gi;
  ...
  stwt.txt.autoLinkCashtags = function (text, options) {
    if (typeof options === 'function') {
      return text.replace(stwt.txt.regexen.cashtag, function (
        match,
        before,
        cashtag
      ) {
